...
# Model parameters
parser.add_argument('--model', default='resnet50', type=str, metavar='MODEL',help='Name of model to train (default: "resnet50"')
...
parser.add_argument('--std', type=float, nargs='+', default=None, metavar='STD',help='Override std deviation of dataset')
parser.add_argument('--interpolation', default='', type=str, metavar='NAME',help='Image resize interpolation type (overrides model)')
parser.add_argument('-b', '--batch-size', type=int, default=128, metavar='N',help='Input batch size for training (default: 128)')
parser.add_argument('-vb', '--validation-batch-size', type=int, default=None, metavar='N', help='Validation batch size override (default: None)')
...
parser.add_argument('--grad-checkpointing', action='store_true', default=False, help='Enable gradient checkpointing through model blocks/stages')
...
parser.add_argument('--weight-decay', type=float, default=1e-4, help='weight decay (default: 2e-5)')
...
parser.add_argument('--decay-milestones', default=[25, 50, 75, 100, 120, 150], type=int, nargs='+', metavar="MILESTONES",  help='list of decay epoch indices for multistep lr. must be increasing')
...
parser.add_argument('--aug-repeats', type=float, default=0, help='Number of augmentation repetitions (distributed training only) (default: 0)')
parser.add_argument('--aug-splits', type=int, default=0, help='Number of augmentation splits (default: 0, valid: 0 or >=2)')
parser.add_argument('--jsd-loss', action='store_true', default=False,help='Enable Jensen-Shannon Divergence + CE loss. Use with `--aug-splits`.')
...
parser.add_argument('--drop-rates', type=json.loads, default={}, metavar='PCT', help='Dropout rates. pass the arguments like \'{"1":0.1, "2":0.1}\' (default: {}.)')
...
parser.add_argument('-j', '--workers', type=int, default=4, metavar='N', help='how many training processes to use (default: 4)')
...
parser.add_argument('--no-ddp-bb', action='store_true', default=False,help='Force broadcast buffers for native DDP to off.')
...
@record
def main():
    #! create logfiles inside each individual experiment directory
    # setup_default_logging(log_path=f'logs.txt')
    args, args_text = _parse_args()

    exp_name = '-'.join([datetime.now().strftime("%Y%m%d-%H%M%S"),safe_model_name(args.model), str('224')])
    output_dir = get_outdir(args.output if args.output else './output/train', exp_name)
    setup_default_logging(log_path=os.path.join(output_dir,'logs.txt'))
    ...
    _logger.info(f"In the name of God the most compassionate the most mericufl")
    _logger.info(f'=> call arguments : {" ".join(sys.argv)}')
    _logger.info(f"=> effective arguments : {args}")
    #Python version!
    _logger.info("=> Python version : {}".format(sys.version.split('\n')[0]))
    _logger.info(f"=> Pytorch version : {torch.__version__}")
    _logger.info(f"=> CUDA Availability : {torch.cuda.is_available()}")
    _logger.info(f"=> CUDA version : {torch.version.cuda}")
    _logger.info(f"=> CuDNN version : {torch.backends.cudnn.version()}")   
    _logger.info(f"=> Initial cuda seed : {torch.cuda.initial_seed()}")
    if torch.cuda.is_available():
        _logger.info(f"=> Available GPUs : {torch.cuda.device_count()}")
        for i in range(torch.cuda.device_count()):
            properties = torch.cuda.get_device_properties(i)
            #CudaDeviceProperties(name='GeForce GTX 1080', major=6, minor=1, total_memory=8116MB, multi_processor_count=20)
            _logger.info("\t=> GPU({}): {} ({},{}) Total memory: {}MB Multi processor count: {}".format(i, 
                                                    properties.name,
                                                    properties.major,
                                                    properties.minor,
    ...
    if args.distributed and args.sync_bn:
        ...
        if args.local_rank == 0:
            _logger.info('Converted model to use Synchronized BatchNorm. WARNING: You may have issues if using '
    ...
    _logger.info(f"=> optimizer : '{str(optimizer)}'")
    ...
    best_epoch = None
    saver = None
    #! todo: changes concerning log file are ineffective here, remove them!
    output_dir_ = None
    if args.rank == 0:
        if args.experiment:
            exp_name_ = args.experiment
        else:
            exp_name_ = '-'.join([datetime.now().strftime("%Y%m%d-%H%M%S"),safe_model_name(args.model), str(data_config['input_size'][-1])])
        
        output_dir_ = get_outdir(args.output if args.output else './output/train', exp_name)
        
        decreasing = True if eval_metric == 'loss' else False
        saver = CheckpointSaver(model=model, optimizer=optimizer, args=args, model_ema=model_ema, amp_scaler=loss_scaler,
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if output_dir is not None:
                update_summary(epoch, train_metrics, eval_metrics, optimizer, os.path.join(output_dir, 'summary.csv'),write_header=best_metric is None, log_wandb=args.log_wandb and has_wandb)
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.local_rank == 0:
                _logger.info('Train: {} [{:>4d}/{} ({:>3.0f}%)]  '
                            ...
                            'Time-Batch: {batch_time.val:.3f}s, {rate:>7.2f}/s  '
                            'LR: {lr:.3e}  '
                ...
                if args.save_images and output_dir:
                    torchvision.utils.save_image(input,
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    '{0}: [{1:>4d}/{2}]  '
                    'Time-Batch: {batch_time.val:.3f} ({batch_time.avg:.3f})  '
                    'Loss: {loss.val:>7.4f} ({loss.avg:>6.4f})  '
                    'Acc@1: {top1.val:>7.4f} ({top1.avg:>7.4f})  '
                    'Acc@5: {top5.val:>7.4f} ({top5.avg:>7.4f})'.format(log_name, batch_idx, last_idx, 
                                                                        batch_time=batch_time_m,
